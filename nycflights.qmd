...
```{r}
#| paged.print: false
# gjennomsnittslig ankomstforsinkelse i May mnd.
arr_delay_may |> 
  group_by(day) |> 
  summarise(
    gjennomsnitt_forsinkelse = round(mean(
    arr_delay, 
    na.rm = TRUE, 
    digits = 2 
  )), 
  total_f = sum(arr_delay, na.rm = TRUE), 
  median.f = median(arr_delay, na.rm = TRUE), 
...
flights <- flights %>%
  ...
  select(month:day, gain, dep_delay, speed, everything()) %>%
  arrange(gain)
head(flights, n = 5)
```
